    ...
    <div class="page animsition">
        <div class="page-header">
            ...
            <div class="page-header-actions">
                <form action="{{ url('/siswa/language/score') }}" method="get" class="form-inline">
                    @if($classrooms)
        ...
        <div class="page-content">
            <div class="panel">
                ...
                <div class="panel-body">
                    @if($students)
                        <form action="{{ url('/siswa/language-student') }}" method="post">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}" >
                            <input type="hidden" name="date" value="{{ $_GET['date'] or '' }}">
                            <input type="hidden" name="language" value="{{ $_GET['bahasa'] or '' }}">
                            ...
                            <table class="table table-striped">
                                ...
                                @foreach($students as $std)
                                    <tr>
                                        <td>{{ $std->nama }}</td>
                                        <td>
                                            <input type="radio" name="attendance[{{ $std->student_id }}]" value="hadir" @if(@$langStudent[$std->student_id]['attendance'] == 'hadir' or @$std->bacaan[0]->attendance == '') checked @endif> Hadir
                                            <input type="radio" name="attendance[{{ $std->student_id }}]" value="izin" @if(@$langStudent[$std->student_id]['attendance'] == 'izin') checked @endif> Izin
                                            <input type="radio" name="attendance[{{ $std->student_id }}]" value="sakit" @if(@$langStudent[$std->student_id]['attendance'] == 'sakit') checked @endif> Sakit
                                            <input type="radio" name="attendance[{{ $std->student_id }}]" value="alpa" @if(@$langStudent[$std->student_id]['attendance'] == 'alpa') checked @endif> Alpa
                                        </td>
                                        <td>
                                            <input type="text" class="form-control" name="score[{{ $std->student_id }}]" value="{{ @$langStudent[$std->student_id]['score'] }}"/>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
